...
function Career(props) {
  return (
    <Motion style={{ marginTop: spring(props.ComponentLoadedState ? 0 : -100, { stiffness: 70 } ), opacity : spring(props.ComponentLoadedState ? 1 : 0, { stiffness: 70 } ) }}>
      {(val) =>
        <CareerWrapper BrowserWidth={props.BrowserWidth} style={{ marginTop: `${val.marginTop}px`, opacity: `${val.opacity}` }}>
          <ol>
            <li>
...
const CareerWrapper = styled.div`
  ${(props) => props.BrowserWidth >= 992 ? 
    css`margin-left: 20px; margin-top: 0px;` : css`margin-top: 20px; margin-left: 0px;`
  }
  margin-left: 20px;
`;
